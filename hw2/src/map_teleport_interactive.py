...
map_limits = None  # Will store (x_limit, y_limit, image_size, data_bounds)
map_display = None
original_map = None
...
def pixel_to_habitat_coords(pixel_x, pixel_y, x_limit, y_limit, image_size, data_bounds=None):
    """
...
def habitat_to_pixel_coords(habitat_x, habitat_z, x_limit, y_limit, image_size, data_bounds=None):
    """
...
def update_map_display(position, x_limit, y_limit, image_size, data_bounds):
    """
    Update the map display with the current agent position.
    
    Args:
        position: Agent's current position as (x, y, z)
        x_limit, y_limit, image_size, data_bounds: Map parameters
    """
...
def mouse_callback(event, x, y, flags, param):
    ...
    """
    global sim, agent, current_position, map_limits, map_display, original_map, floor_height
...
def calculate_data_bounds(map_image_path='map.png', threshold=250):
    """
...
def load_map_limits(pointcloud_path='semantic_3d_pointcloud'):
    """
